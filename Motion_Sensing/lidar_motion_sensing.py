...
'''
Motion Sensing + Collision Alert
...
class lidar_subscriber(object):
    ...
    def sense_motion(self):
        lidar_last = self.lidar_data_queue[-1]
        lidar_wo_last = np.delete(self.lidar_data_queue, -1, axis=0)
        avg_dsts = np.mean(lidar_wo_last, axis=0)

        if np.average(np.abs(lidar_last - avg_dsts)) > self.motion_change:
            # rospy.loginfo(np.average(np.abs(lidar_last - avg_dsts)))
            self.motion_alert = True
        else:
            self.motion_alert = False
    ...
    def listen(self, msg):
        if self.run_once == True:
            ...
            if np.shape(self.lidar_data_queue)[0] <= self.queue_size:
                self.lidar_data_queue = np.vstack([self.lidar_data_queue, self.lidar_data])
            else:
                self.lidar_data_queue = np.vstack([self.lidar_data_queue, self.lidar_data])
                self.lidar_data_queue = np.delete(self.lidar_data_queue, 0, axis=0)
                self.sense_motion()
    ...
    def animate(self, i):
        ...
        if self.motion_alert:
            self.ax.text(-2, -2, 'MOTION ALERT !!!', style='italic', fontsize=20, color="blue")
